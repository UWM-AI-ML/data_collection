...
def find_acquisition_function_maxima(gp, acquisition_function,
                                     ...
                                     optimization_bounds,
                                     acq_func_opt_local_method="L-BFGS-B",
    ...
    if optimization_method == "global":
        ...
        a = minimize(
            ...
            x0,
            args=(gp, acquisition_function, origin, number_of_maxima_sought,cost_function, cost_function_parameters, args),
            method=acq_func_opt_local_method,
        ...
        if a["success"] is False:
            ...
            func_eval = evaluate_acquisition_function(x0,
                                                      gp, acquisition_function, origin = origin,
...
def differential_evolution(ObjectiveFunction,
                           ...
                           args = {},
                           vectorized = True):
    fun = partial(ObjectiveFunction, gp=gp, acquisition_function=acquisition_function, origin=origin, number_of_maxima_sought = number_of_maxima_sought, cost_function=cost_function, cost_function_parameters=cost_function_parameters, args=args)
    updating = 'deferred' if vectorized else 'immediate'
    res = devo(partial(acq_function_vectorization_wrapper, func = fun, vectorized = vectorized), bounds, tol=tol, maxiter=max_iter, popsize=popsize, polish=False, constraints=constraints, updating=updating, vectorized=vectorized)
