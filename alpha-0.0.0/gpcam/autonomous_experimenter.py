...
class AutonomousExperimenterGP():
    ...
    ###################################################################################
    def train(self, pop_size=10, tol=1e-6, max_iter=20, method="global", local_optimizer="L-BFGS-B"):
        """
        ...
        self.gp_optimizer.train_gp(
            ...
            method=method, local_optimizer=local_optimizer, pop_size=pop_size,
    ...
    def go(self, N=1e15, breaking_error=1e-50,
           ...
           retrain_locally_at=(20, 40, 60, 80, 100, 200, 400, 1000),local_optimizer="L-BFGS-B",
           retrain_async_at=(),
           update_cost_func_at=(),
           acq_func_opt_local_method="L-BFGS-B",acq_func_opt_setting=lambda number: "global",
        ...
        while n_measurements < N:
            ...
            res = self.gp_optimizer.ask(
                ...
                bounds=None,
                acq_func_opt_local_method=acq_func_opt_local_method,method=local_method,
            ...
            if any(n in retrain_async_at for n in range(n_measurements,len(self.x_data))) and n_measurements<N:
                ...
                self.train(pop_size=training_opt_pop_size,
                           tol=training_opt_tol,
                           max_iter=training_opt_max_iter,
                           method="local",local_optimizer=local_optimizer)
            else:
